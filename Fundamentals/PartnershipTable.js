...
const PartnershipTable = (props) => {
	...
	return (
		<>
				<text style={{ fontSize: 14, color: 'grey', marginBottom: 10 }}>Partnerships Table</text>
								...
								<tbody>
									{trials.map((trial) => (
										<tr key={Math.random()}>
											...
											<td>{trial?.partnership_with}</td>
											<td>{`${trial?.description.slice(0,50)}...`}</td>
										</tr>
									))}
								</tbody>
							</table>
